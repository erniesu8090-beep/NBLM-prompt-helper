...
6. **【視覺風格絕對一致性】**：為了防止生成風格發散（例如第一組是單色線稿藍圖，第二組卻變成彩色 3D 漸層插圖），你必須在每一頁的 'visual_description' 與 'generation_prompt' 中，使用「排除性且極度具體」的風格關鍵字限制。請觀察第一組所使用的核心視覺語彙，並在後續每一頁的指令中強制鎖定（例如：「僅限單色白線平面藍圖，禁用任何 3D 渲染、禁用漸層色、背景統一為純深色底圖，維持與第一組相同的 corner brackets 點綴」）。
