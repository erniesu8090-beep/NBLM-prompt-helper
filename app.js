...
- 當要求「生成」簡報架構時：根據來源內容的主題，自動定義最適合的 global_design（如：科技主題配深色模式、教育主題配明亮手繪），並將內容拆解為 slides，確保每一頁都有明確的視覺描述 (visual) 與生成提示 (content)。
- 移除所有來源的項目編號，確保內容簡潔。
- 生成的簡報內容不要有字體的名字。
...
slides:
  - page: 1
    visual: "畫面視覺描述 (極簡，20字內)"
    title: "頁面大標題"
    subtitle: "副標題"
    content: "投影片版面文字生成指令 (包含列點重點，極簡，50字內)"
...
7. **【多組檢測提示】**：若你評估來源文件的內容在規劃完本組（最多新增 11 頁新內容）後**仍有剩餘**，請在 YAML 輸出最末端以註解形式標記：\`# ⚠️ 提示：來源文件尚未完全解析完畢，請繼續貼入此提示詞以接續規劃下一組。\`。若已講完，則標記：\`# 🎉 提示：所有內容已全數規劃完畢！\`。
...
slides:
  ...
  - page: [接續的起始頁碼]
    visual: "詳細描述畫面配置 (極簡，20字內)"
    title: "本頁大標題"
    subtitle: "副標題"
    content: "給 AI 的投影片內文生成指令 (如：簡短列點說明，50字內)"
