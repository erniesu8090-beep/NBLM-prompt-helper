...
function init() {
  ...
  const pptBtnStep2 = document.getElementById("ppt-mode-btn-step2");
    ...
    const PPT_PROMPTS = {
    step1: `你是一位頂尖的簡報架構師 (Presentation Architect)。請深入分析目前筆記本中的所有來源文件，並為我規劃「第一組」的簡報架構（第 1 頁至第 8 頁）。你具備以下能力與遵循準則：
...
- 僅輸出「第一組」的簡報規劃，頁數限制為第 1 頁至第 8 頁。
...
- 若來源資訊不足，請根據專業設計邏輯進行「合理推斷」並填入 YAML 中，而非留白。`,
    step2: `你是一位頂尖的簡報架構師 (Presentation Architect)。請根據我們在上一個對話中生成的簡報架構，接續為我規劃**下一組簡報（最多新增 6 頁）**。
...
3. 接續上一組最後一頁的頁碼，往後規劃後續的簡報頁面（例如：上一組結束於第 8 頁，此組即從第 9 頁開始規劃至第 14 頁；若上一組結束於第 14 頁，此組即從第 15 頁規劃至第 20 頁，依此類推）。
...
7. **【多組檢測提示】**：若你評估來源文件的內容在規劃完本組（最多新增 6 頁新內容）後**仍有剩餘**，請在 YAML 輸出最末端以註解形式標記：\`# ⚠️ 提示：來源文件尚未完全解析完畢，請繼續貼入此提示詞以接續規劃下一組。\`。若已講完，則標記：\`# 🎉 提示：所有內容已全數規劃完畢！\`。
